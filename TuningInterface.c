...
volatile unsigned int Lock1Demand = 40 ;			// % demand for first lock position
volatile unsigned int Lock2Demand = 40;			// % demand for second lock position
volatile unsigned int Lock1Duration = 600;		// Lock duration in 1ms increments
volatile unsigned int Lock2Duration = 600;		// Lock duration in 1ms increments
volatile unsigned int RampStartDemand = 55;		// % demand at the beginning of the open-loop starting ramp
volatile unsigned int RampEndDemand = 75;		// % demand at the end of the open-loop starting ramp
volatile unsigned int RampDuration = 4000;		// Ramp duration in 1ms increments
volatile unsigned int RampStartSpeed = 100;		// Speed (in RPM) at the beginning of the open-loop starting ramp
volatile unsigned int RampEndSpeed = 2000;		// Speed (in RPM) at the end of the open-loop starting ramp
volatile unsigned int MaxMotorSpeed = 30000;	// Maximum speed of the motor - used to normalize the potentiometer speed adjustment
volatile unsigned int BlankingCountInit = 6;	// The number of ADC samples "ignored" after commutating the motor windings
volatile unsigned int SpeedControl_P = 120;		// The P term for the PI speed control loop
volatile unsigned int SpeedControl_I = 20;		// The I term for the PI speed control loop
volatile unsigned int SpeedControl_D = 20;		// The I term for the PI speed control loop
volatile unsigned int PhaseAdvanceStart = 5000;	// The RPM at which point phase advance begins to be added
volatile unsigned int PhaseAdvanceSlope = 2; 	// The number of degrees of phase advance per 1000 RPM (after the phase advance start speed is reached) 
volatile unsigned int InitialDemand = 50;		// % demand immediately after the starting ramp is completed.
volatile unsigned int NoOfMotorPoles = 4;		// Number of motor poles (not pole-pairs)
...
volatile unsigned int NumOfTimer1TimeOuts = 5;  // Defines the number of Timer 1 timeouts that will sequentially occur before the motor shuts down
